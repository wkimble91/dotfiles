...
if [ "$COLOR_FOREGROUND" != "" ]; then
    COLOR_FOREGROUND_BEGIN="%{F$COLOR_FOREGROUND}"
    COLOR_FOREGROUND_END="%{F-}"
fi
if [ -z "$CITY_NAME" ]; then
...
function setIcons {
    ...
    if [ "$DISPLAY_WIND" = "yes" ] && [ `echo "$WINDFORCE >= $MIN_WIND" |bc -l` -eq 1 ]; then
        ...
        WIND="$WIND $COLOR_FOREGROUND_BEGIN|$COLOR_FOREGROUND_END"
    fi
...
function outputCompact {
    OUTPUT="$WIND %{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON%{F-}%{T-} $ERR_MSG$COLOR_TEXT_BEGIN$DESCRIPTION$COLOR_TEXT_END $COLOR_FOREGROUND_BEGIN|$COLOR_FOREGROUND_END  $TEMP"
